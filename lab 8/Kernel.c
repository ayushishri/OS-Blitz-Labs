...
function InitFirstProcess ()
	...
	thPtr.Fork(StartUserProcess,"TestProgram5" asInteger)
endFunction
...
function StartUserProcess(arg: int)
	...
	openFile = fileManager.Open("TestProgram5")
		...
		frameManager.GetNewFrames(&(newPCB.addrSpace), currentThread.myProcess.addrSpace.numberOfPages)
			...
			for i = 0 to MAX_FILES_PER_PROCESS-1
				if currentThread.myProcess.fileDescriptor[i] != null
					...
					newThr.myProcess.fileDescriptor[i] = currentThread.myProcess.fileDescriptor[i]
				endIf
			endFor
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      var 
	i,fileDescriptorIndex, oldVar:int
	openFile: ptr to OpenFile
	kernelAddr: array [MAX_STRING_SIZE] of char
	
	
	
      --copy filename string from virt space to a small buffer
      oldVar = currentThread.myProcess.addrSpace.GetStringFromVirtual(&kernelAddr, filename asInteger, MAX_STRING_SIZE)
	fileDescriptorIndex = -1

	--make sure length of filename does not exceed MAX_STRING_SIZE
	

	--uncomment for ReadTest1
    	--printInt(filename arraySize)
	--nl()
	--printInt(MAX_STRING_SIZE)
	--nl()
	--nl()
	...
	--locate empty slot in this process's fileDescriptor array
	...
	--allocate openFile object. if this fails return -1

	
	if StrEqual(&kernelAddr,"terminal")
	...
	--set entry in the fileDescriptor array to point to the OpenFile object	
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	var virtAddr, virtPage, offset, copiedSoFar, nextPosInFile, chunkSize, fileSize, destAddr, i: int
	    openFile: ptr to OpenFile
	    readSuccess: bool
	    c: char
	...
	if currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL
	   -- if the filename argument happens to be the special string “ terminal”, your kernel will not search the disk for a file with that name;
	   -- instead your kernel will return a file descriptor that refers to the serial terminal device.  
	   copiedSoFar = 0
		...
		--check user target area and return from syscall if problems
		...
		while true 
			...
			if nextPosInFile + chunkSize > fileSize
				chunkSize = fileSize - nextPosInFile
			endIf
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	...
	-- instead your kernel will return a file descriptor that refers to the serial terminal device.  
		tbuff = buffer
		for i=0 to sizeInBytes-1
			virtPage = tbuff asInteger / PAGE_SIZE
			virtAddr = tbuff asInteger
		   if (virtPage < 0 || virtPage > currentThread.myProcess.addrSpace.numberOfPages-1 || currentThread.myProcess.addrSpace.IsValid(virtPage) ==false || currentThread.myProcess.addrSpace.IsWritable(virtPage) == false)
		...
		while true 
			...
			currentThread.myProcess.addrSpace.SetDirty(virtPage)
			destAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr(virtPage) + offset
			writeSuccess = fileManager.SynchWrite(openFile, destAddr, nextPosInFile, chunkSize)
			nextPosInFile = nextPosInFile + chunkSize
			copiedSoFar = copiedSoFar + chunkSize
			virtPage = virtPage + 1
			offset = 0
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
	...
	openFile = currentThread.myProcess.fileDescriptor[fileDesc]
	--if newCurrentPos is equal to -1, we set it to N from lab 5
	if newCurrentPos == -1 
...
behavior SerialDriver
	method Init()
		...
		serialHandlerThread.isUserThread = false
	...
	--GetChar() and PutChar methods are similar to producer/consumer functions 
	...
	method PutChar(value:char)
		...
		putBufferNextIn = (putBufferNextIn + 1) % SERIAL_GET_BUFFER_SIZE
	...
	method SerialHandler()
		...
		while true
			...
			--handle input stream
			if (statusReg & SERIAL_CHARACTER_AVAILABLE_BIT) == SERIAL_CHARACTER_AVAILABLE_BIT
				...
				--process character in getBufferSize
			...
			if (statusReg & SERIAL_OUTPUT_READY_BIT) == SERIAL_OUTPUT_READY_BIT
			 	serialLock.Lock()
			 	if putBufferSize > 0
			    		inChar = putBuffer[putBufferNextOut]
					*(serial_data_word_address) = inChar
...
----------------------------- Serial Handler -----------------------------------
  function SerialHandlerFunction(junk:int)
