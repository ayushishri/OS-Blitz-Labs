...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
	...
	var 
	  nexThread: ptr to Thread 
	  oldIntStat: int
	  oldIntStat = SetInterruptsTo (DISABLED)
	  if (heldBy != currentThread)	
	      FatalError("Thread called Unlock() but does not hold mutex lock")
	  endIf
	  heldBy = null
	  nexThread = waitingThreads.Remove()
	  if nexThread != null
	      nexThread.status = READY
	      readyList.AddToEnd (nexThread)
	  endIf
	  oldIntStat = SetInterruptsTo (oldIntStat) 
        endMethod
	...
	if(heldBy == currentThread)
            FatalError ("Thread already has a Lock() and called mutex lock")
          endIf
	while(heldBy != currentThread)
	    oldIntStat = SetInterruptsTo (DISABLED)
            if heldBy == null
